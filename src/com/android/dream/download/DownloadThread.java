	...
	public void run() {
		if(downloadedLength < block){//未下载完成
			try {
				...
				byte[] buffer = new byte[4*1024*1024];	//设置本地数据缓存的大小为1M
				...
				while (!downloader.getExited() && (offset = inStream.read(buffer, 0, 4*1024*1024)) != -1) {	//但用户没有要求停止下载，同时没有到达请求数据的末尾时候会一直循环读取数据
