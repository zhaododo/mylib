...
public class BitmapManager {  
    ...
    private static Map<PersonalHeaderView, String> headerImageViews;
    ...
    static {  
        ...
        headerImageViews = Collections.synchronizedMap(new WeakHashMap<PersonalHeaderView, String>());
    }  
     ...
     * @param imageView
     */
    public void loadBitmap(String url, PersonalHeaderView imageView) {  
    	loadBitmap(url, imageView, this.defaultBmp, 0, 0);
    }
    ...
    public void loadBitmap(String url, ImageView imageView,int width,int height) {  
    	loadBitmap(url, imageView, this.defaultBmp, width, height);
    }
    /**
    ...
    public void loadBitmap(String url, ImageView imageView, Bitmap defaultBmp, int width, int height) {  
        ...
        if (bitmap != null) {  
        	...
        	String filepath = imageView.getContext().getFilesDir() + File.separator + filename+".png";
     ...
     * @param height
     */
    public void loadBitmap(String url, PersonalHeaderView imageView, Bitmap defaultBmp, int width, int height) {  
    	headerImageViews.put(imageView, url);  
        Bitmap bitmap = getBitmapFromCache(url);  
   
        if (bitmap != null) {  
			//显示缓存图片
            imageView.setmSrc(bitmap);  
        } else {  
        	//加载SD卡中的图片缓存
        	String filename = FileUtils.getFileName(url);
        	String filepath = imageView.getContext().getFilesDir() + File.separator + filename+".png";
    		...
    		if(file.exists()){
				//显示SD卡中的图片缓存
    			Bitmap bmp = ImageUtils.getBitmap(imageView.getContext(), filename);
        		imageView.setmSrc(bmp);
        	}else{
				//线程加载网络图片
        		imageView.setmSrc(defaultBmp);
        		queueJob(url, imageView, width, height);
        	}
        }  
    }  
     ...
     * @param height
     */
    public void queueJob(final String url, final PersonalHeaderView imageView, final int width, final int height) {  
        /* Create handler in UI thread. */  
        final Handler handler = new Handler() {  
            public void handleMessage(Message msg) {  
                String tag = headerImageViews.get(imageView);  
                if (tag != null && tag.equals(url)) {  
                    if (msg.obj != null) {  
                        imageView.setmSrc((Bitmap) msg.obj);
        ...
        try {
			...
			if(bitmap != null){
				if(width > 0 && height > 0) {
					...
					if(wid>1500 || hight>900){
						bitmap = ImageUtils.zoomBitmap(bitmap,wid/2,hight/2);
					}else{
						bitmap = Bitmap.createScaledBitmap(bitmap, wid, hight, true);
					}
				} 
				//放入缓存
